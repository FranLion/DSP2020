...
'''------------------------------------------------Primera parte--------------------------------------------------------------'''
#A continución se procede a conseguir un largo de ventana M tal que el filtro deje pasar la componente de 10kHz con una atenuación de 3dB (30%) aproximadamente. Para ello se utiliza la expresión de la respuesta en frecuencia del filtro y se busca calcular los largos L de ventana para los que la magnitud sea nula en la frecuencia angular que corresponde a 10 kHz (w0). Estos largos L_k estarán dados por L_k=(2*π/w0)*k, expresión encontrada de manera analítica. Como se busca el L_1 para el primer cero, se impone k=1.
...
for i in range(0,L):
    ...
    if RF_FMM[i][i_w0]>=0.707 and RF_FMM[i][i_w0]<0.8:
        M=aux
# Se presentan los resultados en una tabla para poder observar el porcentaje de atenuación de cada ventana en 10 kHz. El que cumple la condición de atenuar alrededor de un 30% (en realidad un 24%) es la ventana de largo M=2, por lo que se utiliza este valor para la segunda parte del ejercicio.
